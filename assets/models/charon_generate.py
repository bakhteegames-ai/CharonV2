...
Generates low-poly Charon + Gondola mesh (<=5000 tris) and exports to GLB.
...
M_GLOW   = make_mat('M_Glow',   '#2A905F', emissive=True)      # Ethereal Mint
...
def make_gondola():
    ...
    n = len(profile)
    for i in range(n - 1):
        bm.faces.new([kv[i], lv[i],   lv[i+1], kv[i+1]])
        bm.faces.new([kv[i], kv[i+1], rv[i+1], rv[i]])
        bm.faces.new([lv[i], lv[i+1], rv[i+1], rv[i]])
    bm.faces.new([kv[0],  lv[0],  rv[0]])
    bm.faces.new([kv[-1], rv[-1], lv[-1]])
    bm.normal_update()
...
# -- GOLD TRIM STRIPS ----------------------------------------------------------
# Python 3.11 safe: no quotes inside f-string

trim_objects = []
for y_sign in (-1, 1):
    side_name = 'L' if y_sign < 0 else 'R'
    bpy.ops.mesh.primitive_cube_add(size=1, location=(0, y_sign * 0.45, 0.26))
    t = bpy.context.active_object
    t.name = 'Trim_' + side_name
    t.scale = (2.2, 0.04, 0.03)
    apply_tf(t)
    set_mat(t, M_TRIM)
    trim_objects.append(t)
...
children = ([gondola, body, hood, peak, shaft, blade, glow, arm_l, arm_r]
            + shoulders + trim_objects)
...
print('[charon_generate] Exported -> ' + out_path)
