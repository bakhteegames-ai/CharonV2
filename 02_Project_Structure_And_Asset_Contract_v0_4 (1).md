# Project Structure and Asset Contract v0.4

## 1. Зачем нужен этот файл
Этот файл должен объяснять любому ИИ и любому человеку:
- где лежит код
- где должны лежать runtime-ассеты
- где должны лежать source-ассеты
...
### Важно
В текущем архиве **ещё нет папки `/public`**, но код уже ожидает её существование.

Нужно создать и поддерживать:
...
Если не хочется добавлять отдельные папки в репозиторий прямо сейчас, хотя бы фиксировать их существование в документации и потом создавать по мере наполнения.
...
Если загрузка падает, collider остаётся fallback-визуалом.
...
- Up axis: +Y
- forward direction: **-Z**
- origin желательно около центра персонажа по X/Z
...
- SFX: 0.1–1.5 сек
- ambience: seamless loop
- нормализованный loudness без клиппинга
- избегать слишком длинных и тяжёлых файлов

---

## 6.3 Textures
**Runtime path:** `public/textures/...`

### Статус
Пока не wired напрямую, но контракт нужно зафиксировать уже сейчас.

### Рекомендуемые форматы
- `.webp` или `.png`
- world textures: 1024–2048 max
- UI icons: 256–1024 depending on usage
- mobile-sensitive pipeline: не тащить бессмысленно большие картинки

---

## 6.4 UI / comic overlays
**Runtime path:** `public/ui/...`

### Пока planned
Сюда логично будут идти:
- logo
- button icons
- pause / game over art
- comic splash overlays
- tutorial gesture cards

### Форматы
- `.png` для alpha
- `.webp` для лёгких bitmap assets
- `.svg` допустим для simple vector icons
...
- desktop screenshots
- mobile screenshots
- short trailer / gif previews

---

## 7. Как агенту понимать, что где лежит

Для каждого ассета должен быть **manifest entry**:
...
notes: first animation clip autoplays
```
